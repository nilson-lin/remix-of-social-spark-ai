...
import { Sparkles, Zap, Image, MessageSquare, Target, Layers, Clock, Palette, Upload, Video, Wand2, Shield } from 'lucide-react';
const features = [{
  icon: Sparkles,
  title: 'IA Avançada',
...
export function Features() {
  return <section className="py-24 px-4" id="features">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          delay: index * 0.05
              ...
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-colors ${feature.color === 'primary' ? 'bg-primary/10 group-hover:bg-primary/20' : feature.color === 'secondary' ? 'bg-secondary/10 group-hover:bg-secondary/20' : 'bg-accent/10 group-hover:bg-accent/20'}`}>
                <feature.icon className={`w-6 h-6 ${feature.color === 'primary' ? 'text-primary' : feature.color === 'secondary' ? 'text-secondary' : 'text-accent'}`} />
              </div>
